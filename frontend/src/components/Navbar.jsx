...
export default function Navbar() {
  ...
  const handleNavClick = (path) => { setMenuOpen(false); if (path) navigate(path); };
  const handleLogout   = () => { dispatch(logoutUser()); setUserMenuOpen(false); navigate("/"); };

  return (
    <>
      <style>{`
        .nav-link { position:relative; color:white; cursor:pointer; font-size:clamp(0.9rem,1.5vw,1.05rem); font-weight:500; opacity:0.92; transition:opacity 0.2s; white-space:nowrap; }
        .nav-link::after { content:''; position:absolute; bottom:-3px; left:50%; right:50%; height:2px; background:white; transition:left 0.25s ease,right 0.25s ease; }
        .nav-link:hover::after { left:0; right:0; }
        .nav-link:hover { opacity:1; }
        @media (max-width:768px) { .desktop-links{display:none;} .mobile-menu{display:flex;} }
        @media (min-width:769px) { .hamburger{display:none;} .mobile-menu{display:none;} }
      `}</style>
      ...
      <nav className="w-full bg-red-600 relative" style={{ fontFamily:"Roboto, sans-serif" }}>
        <div className="flex items-center w-full px-4 sm:px-6 py-4">
          ...
          <div className="flex items-center gap-3 cursor-pointer shrink-0"
            ...
            <span className="text-white font-bold text-xl whitespace-nowrap">Heart Predict</span>
          </div>

          {/* Desktop links */}
          <ul className="desktop-links flex items-center justify-center gap-6 lg:gap-10 list-none m-0 p-0 flex-1 px-4">
            {NAV_LINKS.map((item) => (
              <li key={item.label} className="nav-link" onClick={() => handleNavClick(item.path)}>
                {item.label}
              </li>
            ))}
          </ul>
          ...
          <div className="flex items-center gap-3 shrink-0 ml-auto relative">
            ...
            {/* Hamburger */}
            <button className="hamburger flex flex-col justify-center items-center w-9 h-9 gap-1.5 rounded-md hover:bg-red-500 transition-colors"
        ...
        {/* Mobile dropdown */}
        <div className="mobile-menu flex-col bg-red-700 overflow-hidden transition-all duration-300"
